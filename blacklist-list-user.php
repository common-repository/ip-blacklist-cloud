...
if(isset($_GET['del']))
{
	...
	if($USER_ID && is_numeric($USER_ID))
	{
		...
		$USER=$wpdb->get_var($wpdb->prepare("SELECT USERNAME FROM ".$prefix_tablex."IPBLC_usernames WHERE id=%d",$USER_ID));


		if($USER)
		{
			$wpdb->query($wpdb->prepare("DELETE FROM ".$prefix_tablex."IPBLC_usernames WHERE id=%d",$USER_ID));
		}
...
post_blacklist_remove_user($USER2);
...
else if($orderby=="visits")
{
	...
	global $wpdb,$prefix_tablex;
		...
		$totalUSER = $wpdb->query("SELECT * FROM ".$prefix_tablex."IPBLC_usernames ORDER BY $orderby $order");

		$resultX = $wpdb->get_results("SELECT * FROM ".$prefix_tablex."IPBLC_usernames ORDER BY $orderby $order LIMIT $offset, $rowsPerPage");
...
<td class="name column-name">
	...
	<a href="https://www.ip-finder.me/wpuser?user=<?php echo $USER2; ?>" target="_blank" title="IP Details on IP-Finder.me">Details on IP Blacklist Cloud</a>
