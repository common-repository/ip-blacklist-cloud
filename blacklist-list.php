...
else if($orderby=="lastvisit")
{
	...
	global $wpdb,$prefix_tablex;
		...
		$totalIP = $wpdb->query("SELECT * FROM ".$prefix_tablex."IPBLC_blacklist ORDER BY $orderby $order");

		$resultX = $wpdb->get_results("SELECT * FROM ".$prefix_tablex."IPBLC_blacklist ORDER BY $orderby $order LIMIT $offset, $rowsPerPage");
		...
		{
			$ss=$s_value;
		$sql1=$wpdb->prepare("SELECT * FROM ".$prefix_tablex."IPBLC_blacklist WHERE IP LIKE %s ORDER BY $orderby $order","%$ss%");



		$totalIP = $wpdb->query($sql1);

		$resultX = $wpdb->get_results( $wpdb->prepare("SELECT * FROM ".$prefix_tablex."IPBLC_blacklist  WHERE IP LIKE %s ORDER BY $orderby $order LIMIT $offset, $rowsPerPage","%$ss%"));
