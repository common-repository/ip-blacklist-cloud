...
$totalIP = $wpdb->query($wpdb->prepare("SELECT * FROM ".$prefix_tablex."IPBLC_login_failed  WHERE IP=%s  ORDER BY timestamp desc",$IP));


$resultX = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".$prefix_tablex."IPBLC_login_failed  WHERE IP=%s ORDER BY timestamp desc LIMIT $offset, $rowsPerPage",$IP));
