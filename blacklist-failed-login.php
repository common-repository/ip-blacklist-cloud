...
<?php
	...
	if(isset($_POST['emptyFailed']))
	{
		...
		$wpdb->query("TRUNCATE TABLE ".$prefix_tablex."IPBLC_login_failed ");
...
else if($orderby=="blc")
{
	...
	global $wpdb,$prefix_tablex;
		...
		$totalIP = $wpdb->query("SELECT DISTINCT(IP), id, COUNT(IP) as countx,timestamp FROM ".$prefix_tablex."IPBLC_login_failed  GROUP BY IP  ORDER BY $orderby $order");
 ...
 FROM ".$prefix_tablex."IPBLC_login_failed GROUP BY IP ORDER BY $orderby $order LIMIT $offset, $rowsPerPage");
...
<td class="username column-username"><a href="https://www.ip-finder.me/<?php echo $this_IP->IP; ?>/" title="<?php echo $this_IP->IP; ?>"><?php echo $this_IP->IP; ?></a></td>
...
<td class="name column-name">
	<?php
		$ippXX=$this_IP->IP;
		$singleDetails = $wpdb->get_results( "SELECT variables FROM ".$prefix_tablex."IPBLC_login_failed WHERE IP=\"$ippXX\" ORDER BY timestamp DESC");
		if($singleDetails)
		{
...
<?php
		...
		$failedID=$this_IP->id;
			...
			$IP_in_DP=$wpdb->get_var("SELECT id FROM ".$prefix_tablex."IPBLC_blacklist WHERE IP='$IP'");
			if($IP_in_DP)
			{
