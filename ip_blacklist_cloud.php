...
if ( !defined('ABSPATH') )
	...
	global $wpdb,$prefix_tablex;
  	...
  	$prefix_tablex=$wpdb->prefix;
  	if (is_multisite())
  	{
    	$prefix_tablex=$wpdb->base_prefix;
	}  
...
function ip_added()
{
	global $wpdb,$prefix_tablex,$found,$IP_global, $IP_error;

	$IP=sanitize_text_field($_GET['blacklist']);
	$table=$prefix_tablex."IPBLC_blacklist";
...
function user_added()
{
	global $wpdb,$prefix_tablex,$found,$USER_global, $USER_error;

	$USER=$_POST['blacklistuser'];
	$USER=sanitize_text_field($USER);

	$USER_in_DB=$wpdb->get_var($wpdb->prepare("SELECT id FROM ".$prefix_tablex."IPBLC_usernames WHERE USERNAME=%s",$USER));
	$found=false;
	...
	if(!$USER_in_DB)
	{
		if($USER)
		{
			$table=$prefix_tablex."IPBLC_usernames";
			$time=time();
			...
			//----do not send to ip-finder.me in version 5.0---
		}
		else
		{
			$found=true;
...
{  
	...
	global $wpdb,$prefix_tablex;
	$newFailed=0;
	$table1=$prefix_tablex."IPBLC_login_failed";
	$table2= $prefix_tablex."IPBLC_blacklist";
	$countMain="";
	$countOld="";
	$time=time();
...
function IPBLC_IP_value( $column, $comment_ID )
{
	...
	if ( 'IPBLC_IP_status' == $column )
	{
		...
		$IP_in_DP=$wpdb->get_var($wpdb->prepare("SELECT id FROM ".$prefix_tablex."IPBLC_blacklist WHERE IP=%s",$IP));
...
*/
		...
		$USERNAME_in_DP=$wpdb->get_var($wpdb->prepare("SELECT id FROM ".$prefix_tablex."IPBLC_usernames WHERE USERNAME=%s",$authorName2));
...
function create_sql()
{
	...
	if(!($wpdb->query("SELECT * FROM ".$prefix_tablex."IPBLC_login_failed"))) 
	{

		$sql = "CREATE TABLE ".$prefix_tablex."IPBLC_login_failed (
	...
	if(!($wpdb->query("SELECT * FROM ".$prefix_tablex."IPBLC_blacklist"))) 
	{

		$sql = "CREATE TABLE ".$prefix_tablex."IPBLC_blacklist (
		id INT(60) UNSIGNED NOT NULL AUTO_INCREMENT,
		IP VARCHAR(25) NOT NULL, 
		timestamp INT( 30 ) NOT NULL,
	...
	if(!($wpdb->query("SELECT * FROM ".$prefix_tablex."IPBLC_usernames"))) 
	{

		$sql = "CREATE TABLE ".$prefix_tablex."IPBLC_usernames (
		id INT(60) UNSIGNED NOT NULL AUTO_INCREMENT,
...
$link="https://www.ip-finder.me/lastUpdate.php?website=".urlencode(site_url());
	...
	if(filter_var($IP, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4))
	{
		...
		if($IP_in_DP)
		{
			$visits=$wpdb->get_var($wpdb->prepare("SELECT visits FROM ".$prefix_tablex."IPBLC_blacklist WHERE id=%d",$IP_in_DP));
			$visits=$visits+1;

			$t=time();

			$wpdb->query($wpdb->prepare("UPDATE ".$prefix_tablex."IPBLC_blacklist SET `visits`=%d WHERE id=%d",array($visits,$IP_in_DP)));
			$wpdb->query($wpdb->prepare("UPDATE ".$prefix_tablex."IPBLC_blacklist SET `lastvisit`=%d WHERE id=%d",array($t,$IP_in_DP)));
...
<div id="IPBLC_message_blacklist">Your IP <?php echo $IP; ?> has been blacklisted!</div><BR>
	...
	if(isset($_POST['author']))
	{
		...
		$visits=$wpdb->get_var($wpdb->prepare("SELECT visits FROM ".$prefix_tablex."IPBLC_usernames WHERE id=%d",$USER_in_DB));
		$visits=$visits+1;
		$t=time();

		$wpdb->query($wpdb->prepare("UPDATE ".$prefix_tablex."IPBLC_usernames SET `visits`=%d WHERE id=%d",array($visits,$USER_in_DB)));
		$wpdb->query($wpdb->prepare("UPDATE ".$prefix_tablex."IPBLC_usernames SET `lastvisit`=%d WHERE id=%d",array($t,$USER_in_DB)));
...
<div id="IPBLC_message_blacklist"> "<?php echo $USER; ?>" is blacklisted!</div><BR>
    ...
    if(isset($_GET['callback']))
    {
		...
		if($_GET['action']=="getFailedPagination")
		{
	    		...
	    		if($IPBLC_cloud_password && $IPBLC_cloud_on==2 && $IPBLC_cloud_password==$pwd)
			{
				...
				$totalIP = $wpdb->query("SELECT DISTINCT(IP), id, COUNT(IP) as countx,timestamp FROM ".
					$prefix_tablex."IPBLC_login_failed  GROUP BY IP  ORDER BY timestamp DESC");

				$resultX = $wpdb->get_results("SELECT  DISTINCT(IP), id, COUNT(IP) as countx, timestamp 
					FROM ".$prefix_tablex."IPBLC_login_failed GROUP BY IP ORDER BY timestamp DESC LIMIT $offset, $rowsPerPage");
		...
		if($_GET['action']=="getFailedPaginationSingle")
		{
			...
			if($IPBLC_cloud_password && $IPBLC_cloud_on==2 && $IPBLC_cloud_password==$pwd)
			{
				...
				if(filter_var($IP, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4))
				{
					...
					$totalIP = $wpdb->query($wpdb->prepare( "SELECT * FROM ".$prefix_tablex."IPBLC_login_failed WHERE 
					IP=%s GROUP BY IP  ORDER BY  timestamp DESC",$IP));

					$resultX = $wpdb->get_results($wpdb->prepare("SELECT *  FROM ".$prefix_tablex."IPBLC_login_failed WHERE 
					...
					foreach($resultX as $sComment)
					{
						...
						foreach($sComment as $k=>$v)
						{
							...
							if($k=="comment_author_IP")
							{
								...
								$IP_in_DP=$wpdb->get_var($wpdb->prepare("SELECT id FROM ".$prefix_tablex."IPBLC_blacklist 
										WHERE IP=%s",$v));
								if($IP_in_DP)
								{
									$status="<b style=\"color:#FF0000\"> Blacklisted</b>";
		...
		if($_GET['action']=="getFailedData")
		{
			...
			if($IPBLC_cloud_password && $IPBLC_cloud_on==2 && $IPBLC_cloud_password==$pwd)
			{
				...
				$resultX = $wpdb->get_results( "SELECT  DISTINCT(IP), id, COUNT(IP) as countx, timestamp FROM ".
						$prefix_tablex."IPBLC_login_failed GROUP BY IP ORDER BY timestamp DESC LIMIT $offset, $rowsPerPage");
		...
		if($_GET['action']=="getBlacklist")
		{
			...
			{
				$result['verify']="1";
				$IPs_in_DP=$wpdb->get_results("SELECT IP FROM ".$prefix_tablex."IPBLC_blacklist ORDER BY id DESC");
				$IPx=array();
				$userxx=array();
				if($IPs_in_DP)
				{
			...
			if($IPBLC_cloud_password && $IPBLC_cloud_on==2 && $IPBLC_cloud_password==$pwd)
			{
				...
				if(filter_var($IP, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4))
				{
					...
					if($IP_in_DP)
					{
						$table=$prefix_tablex."IPBLC_blacklist";
						$time=time();
						$wpdb->query($wpdb->prepare("DELETE FROM ".$prefix_tablex."IPBLC_blacklist WHERE IP=%s",$IP));
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action']=="blacklistUSERJS")
		{
			if(current_user_can( 'manage_options' ))
			{
				...
				$sql=$wpdb->prepare("SELECT id FROM ".$prefix_tablex."IPBLC_usernames WHERE USERNAME=%s",$USER);
	...
	if(isset($_REQUEST['action']))
	{
		...
		if($_REQUEST['action']=="exportIPCloud")
		{
			if(current_user_can( 'manage_options' ))
			{
				...
				$USERs_in_DP=$wpdb->get_results("SELECT USERNAME FROM ".$prefix_tablex."IPBLC_usernames ORDER BY id DESC");
				if($USERs_in_DP)
				{
...
register_activation_hook( __FILE__, 'create_sql' );
add_action('admin_init', 'ipblc_init_admin');
...
function IPBLC_login_failed(){
  ...
  if(isset($_SERVER['HTTP_USER_AGENT']))
  {
	...
	if($isIPSafe==0)
	{
		...
		$table=$prefix_tablex."IPBLC_login_failed";
...
function check_my_whitelist()
{
    ...
    if($is_myip_safe==0 && is_admin() && !isset($_GET['addMyIp']) && $current_page!="wp-IPBLC-whitelist" && (filter_var($my_ip_now, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4)))
    {
